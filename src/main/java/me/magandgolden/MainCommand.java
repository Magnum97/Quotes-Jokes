...
public class MainCommand extends BaseCommand {
	...
	private static void onQuote (CommandSender sender, @Default ("send") String action, @Optional String numberOrText) {
		...
		// Create new instnace of dataworks and tells it we are using quoteFile
		...
		switch (action) {
			...
			case "add":
				...
				if (! dw.userOverLimit(uuid, "quotes")) // Check how many quotes a user has added
					dw.addFile(sender, uuid, numberOrText);
				else {
					sender.sendMessage(ChatColor.translateAlternateColorCodes('&', // New line is ignored
							"&fPlease remove a quote before adding more")); // Java continues reading until ;
					return;
				}
				break;
			case "list":
